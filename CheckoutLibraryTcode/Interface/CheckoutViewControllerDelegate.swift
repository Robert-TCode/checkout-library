...
@objc public protocol CheckoutViewControllerDelegate: AnyObject {
    ...
    @objc func didGeneratePaymentToken(_ token: String, viewController: CheckoutViewController)

    /// Called when a payment token generation fails.
    /// - Parameter error: An error describing the reason of the failure.
    @objc func tokenGenerationFailed(with error: Error)
}
